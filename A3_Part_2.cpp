...
int main() {
    ...
    while (numberOfCardEachPlayer < 4)
    {
        ...
        for (int i = 0; i < gamebot->getPlayerVector().size(); i++)
        {
            ...
            //----------------------------------------------------
            //strategy of diff card-drawing bias for diff players
            Context* context = new Context(new GreedyPlayer);
            int res = context->DoStrategy(gamebot->getHand());
            cout<<"Greedy player get card num: "<<res<<endl;
            context->set_Strategy(new ModeratePlayer);
            res = context->DoStrategy(gamebot->getHand());
            cout<<"Moderate player get card num: "<<res<<endl;
